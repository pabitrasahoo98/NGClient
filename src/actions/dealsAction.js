...
export const fetchDod =() =>
  async (dispatch) => {
      ...
      let link = `https://ng-server.vercel.app/api/v1/dodproducts`;
...
//saving de
  ...
  async (dispatch) => {
      ...
      let link = `https://ng-server.vercel.app/api/v1/deproducts`;
      const { data } = await axios.get(link);
...
async (dispatch) => {
    ...
    let link = `https://ng-server.vercel.app/api/v1/tdproducts`;
    const { data } = await axios.get(link);
